...
// ------------------------------ Метки ---------------------------------------    
    ...
    // TODO Проверяет наличие креста у данной позиции (центр предпологаемого креста)
    public boolean isCross(Point start){

        boolean isCrossFinished = false;
        Player startPlayer = null;
        Direction direct = Direction.northEast();

        // Определяем первую метку и соответствующего ей игрока
        Point pos = new Point(start);
        Label l = label(pos);

        isCrossFinished = (l == null);
        if(!isCrossFinished)
        {
            startPlayer = label(pos).player();
        }
        ...
        for (int i=0; i<4 && !isCrossFinished && containsRange(pos); i++)
        {
            l = label(pos);
            isCrossFinished = (l == null || !l.belongsTo(startPlayer));
